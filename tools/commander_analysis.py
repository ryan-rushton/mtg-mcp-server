...
commander_analysis_server: FastMCP = FastMCP("MTG Commander Analysis Server", dependencies=["httpx"])


def _categorize_cards_automatically(found_cards: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    for card_data in found_cards:
        ...
        if (any(keyword in oracle_text for keyword in [
            "add", "mana", "search", "basic land", "rampant growth", "cultivate",
            "sol ring", "signet", "talisman", "arcane signet", "chromatic lantern"
        ]) and not any(keyword in oracle_text for keyword in ["draw", "destroy", "counter"])):
            categories["ramp"].append(name)
        
        # Card advantage - draw and advantage
        elif any(keyword in oracle_text for keyword in [
            "draw", "card", "hand", "library", "exile.*play", "impulse", "scry 2 or more"
        ...
        # Targeted disruption - single target removal/interaction
            ...
            "destroy target", "exile target", "counter target", "return target",
            "bounce", "remove", "path to exile", "swords to plowshares"
        ]):
            categories["targeted_disruption"].append(name)
        
        # Mass disruption - board wipes and mass effects
        elif any(keyword in oracle_text for keyword in [
            "destroy all", "exile all", "return all", "wrath", "board wipe",
            "each player", "each opponent", "all creatures", "all artifacts"
        ]):
...
async def analyze_commander_deck(
    ...
    categories: Dict[str, Dict[str, Any]] = {
        "Ramp": {"cards": categorized_cards["ramp"], "target": cz.ramp_target, "target_range": f"{cz.ramp_target}-{cz.ramp_optimal}+", "min_target": cz.ramp_target, "optimal_target": cz.ramp_optimal},
        "Card Advantage": {"cards": categorized_cards["card_advantage"], "target": cz.card_advantage_target, "target_range": f"{cz.card_advantage_target}+", "min_target": cz.card_advantage_target, "optimal_target": cz.card_advantage_optimal},
        "Targeted Disruption": {"cards": categorized_cards["targeted_disruption"], "target": cz.targeted_disruption_target, "target_range": str(cz.targeted_disruption_target), "min_target": cz.targeted_disruption_target, "optimal_target": cz.targeted_disruption_target},
        "Mass Disruption": {"cards": categorized_cards["mass_disruption"], "target": cz.mass_disruption_target, "target_range": str(cz.mass_disruption_target), "min_target": cz.mass_disruption_target, "optimal_target": cz.mass_disruption_target},
        "Lands": {"cards": categorized_cards["lands"], "target": cz.lands_target, "target_range": str(cz.lands_target), "min_target": cz.lands_target - 2, "optimal_target": cz.lands_target},
        "Plan Cards": {"cards": categorized_cards["plan_cards"], "target": cz.plan_cards_target, "target_range": f"~{cz.plan_cards_target}", "min_target": cz.plan_cards_target - 5, "optimal_target": cz.plan_cards_target + 5}
    }
    ...
    analysis_result = {
        ...
        "categories": {},
        "balance_assessment": {
            "overall_score": "excellent" if categories_meeting_min == total_categories 
                            else "good" if categories_meeting_min >= total_categories - 1
        ...
        "recommendations": {
            ...
            "card_overlap_status": "good" if total_categorized_cards > total_cards * 1.2 
                                  else "low" if total_categorized_cards < total_cards * 1.1 
                                  else "moderate"
        },
    ...
    for category_name, data in categories.items():
        ...
        status = "optimal" if count >= optimal_target else "adequate" if count >= min_target else "insufficient"
