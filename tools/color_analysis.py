...
color_analysis_server: FastMCP = FastMCP("MTG Color Analysis Server", dependencies=["httpx"])
...
async def analyze_color_identity(card_names: List[str]) -> str:
    ...
    if colorless_count > 0:
        color_combinations["Colorless"] = {
            ...
            "percentage": round((colorless_count / total_cards) * 100, 1) if total_cards > 0 else 0
        }
    
    sorted_colors = sorted(color_counts.items(), key=lambda x: (-x[1], x[0]))
    for color_combo, count in sorted_colors:
        color_names = [color_map[c] for c in color_combo]
        color_display = "/".join(color_names) if len(color_names) > 1 else color_names[0]
    ...
    analysis_result = {
        "summary": {
            ...
            "color_diversity": len(individual_colors),  # Number of different colors present
            "multicolor_cards": sum(1 for combo in color_counts.keys() if len(combo) > 1)
        },
...
async def analyze_mana_requirements(card_names: List[str]) -> str:
    ...
    coverage_ratio = total_mana_sources / total_color_requirements if total_color_requirements > 0 else 0
    overall_status = "excellent" if coverage_ratio >= 0.5 else "moderate" if coverage_ratio >= 0.3 else "insufficient"
    ...
    for color_code in ["W", "U", "B", "R", "G"]:
        ...
        if spell_req > 0 or land_prod > 0:
            ...
            if spell_req > 0:
                ...
                if land_prod == 0:
                    status = "no_sources"
                    recommendations.append(f"Add {color_name} mana sources - currently have none!")
                elif color_ratio < 0.3:
                    status = "insufficient"
                    recommendations.append(f"Add more {color_name} sources (need {max(1, int(spell_req * 0.5 - land_prod))} more)")
    ...
    if land_total < total_cards * 0.35:  # Less than 35% lands
        recommendations.append(f"Consider adding more lands (current: {land_total}, suggested: {int(total_cards * 0.4)})")
